Shader "KelvinvanHoorn/Skybox"
{
    Properties
    {
        ...
        _ConstellationCubeMap ("Constellation cube map", Cube) = "blank" {}
        _ConstellationColor ("Constellation color", Color) = (0,0.3,0.6,1)
        _NightSkyColor ("Night sky color", Color) = (0,0,0,1)
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 Fragment (v2f IN) : SV_TARGET
            {
                ...
                if (_TimeOfDay < 0.25) {
                    // 从午夜到日出，逐渐变透明
                    _NightSkyColor = lerp(_NightSkyColor, 0, smoothstep(0.0, 0.25, _TimeOfDay));
                }
                else if (_TimeOfDay > 0.75) {
                    // 从日落到午夜，逐渐变不透明
                    _NightSkyColor = lerp(0, _NightSkyColor, smoothstep(0.75, 1.0, _TimeOfDay));
                }
                else {
                    // 白天时完全透明
                    _NightSkyColor = 0;
                ...
                float3 skyColor = sunZenithColor + vzMask * viewZenithColor + svMask * sunViewColor + _NightSkyColor;
                //float3 skyColor = NightSkyColor;
                ...
                float3 moonColor = moonMask * moonNdotL * exp2(_MoonExposure) * moonTexture*0;
                //I cancel the mooncolor
                ...
                constColor *= (1 - sunMask) * (1 - moonMask) * starStrength*0.1;
